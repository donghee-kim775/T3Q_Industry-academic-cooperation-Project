...
//목록보기
function fnList(){
	document.location.href="/admin/banners/"+fnSysMappingCode()+"selectPageListBannersMgt.do";
}
...
$(function(){
	...
	if ("${sysMappingCode}" == "") {
		$("select[name=sys_code]").html("${CacheCommboUtil:getComboStr(UPCODE_SYS_CODE, 'CODE', 'CODE_NM', requestScope.param.sch_sys_code, '시스템')}");
	} else {
		$("select[name=sys_code]").html("${CacheCommboUtil:getEqulesComboStr(UPCODE_SYS_CODE, requestScope.param.sys_mapping_code, 'CODE', 'CODE_NM', requestScope.param.sch_sys_code, '')}");
	}
...
function addFileList(fIndex, fileName, fileSize, uploadZoneNo){
	...
	html += "	</td>"
	html += "	<td class='text-center'>";
	html +=	"	<button type='button' class='btn bg-gradient-danger btn-sm' onclick='deleteFileMulti(" + fIndex +","+ uploadZoneNo + "); return false;'>삭제</button>";
	html += "	</td>"
	html += "</tr>"
...
function uploadFileAjax(){
	...
	$.ajax({
		url:"/admin/banners/"+fnSysMappingCode()+"insertBannersImageAjax.do",
		data:formData,
		type:'POST',
									...
									<tbody>
										<tr>
											...
											<td>
												<select name="zone_code" class="form-control input-sm">
													${CacheCommboUtil:getComboStr(UPCODE_BANNER_ZONE, 'CODE', 'CODE_NM', '', '배너구역')}
												</select>
											</td>
										</tr>
										<tr>
											...
											<td colspan="3">
												<input type="text" class="form-control" name="banner_nm" maxlength="50" />
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td colspan="3">
												...
												<div class="table-responsive">
													<table class="table mt-1" id="fileTableTbody_1">
														<colgroup>
										...
										<tr>
											...
											<td colspan="3">
												...
												<div class="table-responsive">
													<table class="table mt-1" id="fileTableTbody_2">
														<colgroup>
															<col style="width:60px;">
															<col style="width:15%">
															<col style="width:130px;">
