...
function validateReferralCode(referralCode) {
    return new Promise(function(resolve, reject) {
        $.ajax({
            url: '/admin/validateReferralCodeAjax.do',
            type: 'POST',
            ...
            error: function() {
                reject(new Error('추천 코드 검증에 실패했습니다.'));
            }
        });
    });
}
...
async function fnRegister(){
   ...
   // 추천 코드 검증 추가
    var referralCode = $("#r_referral_code").val();
    var isValidReferral = await validateReferralCode(referralCode);
    if (!isValidReferral) {
        alert("올바른 추천 코드를 입력해주세요.");
        $("#r_referral_code").focus();
        return;
    }
        ...
        $.ajax({
            ...
            success : function(response){
                ...
                if(response.resultMsg == "success"){
                    ...
                    return;
                } else if(response.resultMsg == "invalid_referral_code") {
                    alert("올바른 추천 코드를 입력해주세요.");
                    $("#r_referral_code").focus();
                    return;
                } else {
               ...
               <table class="table table-bordered">
                  ...
                  <tbody>
                     ...
                     <tr>
                        <th>추천 코드*</th>
                        <td>
                           <input type="text" class="form-control" name="r_referral_code" id="r_referral_code" maxlength="20" placeholder="추천 코드"/>
                        </td>
                     </tr>
                  </tbody>
               </table>
