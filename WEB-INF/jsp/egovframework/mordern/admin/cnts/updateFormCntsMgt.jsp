...
//<![CDATA[
	...
	function fnDetail(){
		$("#aform").attr({action:"/admin/cnts/"+fnSysMappingCode()+"selectCntsMgt.do", method:'post'}).submit();
	}
	...
	function fnUpdate(){
		...
		if(confirm("수정하시겠습니까?")){
			...
			$.ajax({
				url:"/admin/cnts/"+fnSysMappingCode()+"updateCntsMgt.do",
				data:formData,
				type:'POST',
	...
	function fnVerChange(){
		if(confirm("예전 버전을 가져오시겠습니까?")){
		...
		<div class="row">
			<div class="col-lg-12">
				...
				<form role="form" id="aform" name="aform" method="post" enctype="multipart/form-data">
					<input type="hidden" name="cntnts_seq"		value="${requestScope.param.cntnts_seq}"/>
					...
					<input type="hidden" name="last_ver"		value="${requestScope.param.last_ver}"/>
								...
								<table class="table text-nowrap inlineBlock">
									...
									<tbody>
										<tr>
											<th>버전 정보</th>
											<td>
												<select name="select_ver" class="form-control w-25" id="select_ver" onchange="fnVerChange()">
												<c:forEach items="${versList}" var="list">
													<option value="${list.VER}" <c:if test="${list.VER eq resultMap.VER}" >selected="selected" </c:if>>Ver.${list.VER}</option>
												</c:forEach>
												</select>
											</td>
										</tr>
										...
										<tr>
											<th>텍스트 내용</th>
											<td>
												<textarea class="form-control" rows="3" name="text_cn" placeholder="텍스트 내용">${resultMap.TEXT_CN}</textarea>
											</td>
										</tr>
										<tr>
											<th>스크립트</th>
											<td>
												<textarea class="form-control" rows="5" name="script" placeholder="스크립트">${resultMap.SCRIPT}</textarea>
											</td>
										</tr>
										<tr>
											<th>키워드</th>
											<td>
												<textarea class="form-control" rows="3" name="kwrd" placeholder="키워드" onkeyup="cfLengthCheck('키워드는', this, 2000);">${resultMap.KWRD}</textarea>
											</td>
										</tr>
										<tr>
											<th>첨부파일 목록</th>
