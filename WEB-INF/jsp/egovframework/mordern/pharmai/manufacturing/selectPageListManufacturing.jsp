...
//<![CDATA[
	...
	function fnGoPage(currentPage){
		...
		$("#aform").attr({action:"/pharmai/chemical/manufacturing/selectPageListManufacturing.do", method:'post'}).submit();
	}
	...
	function fnGoProject(prjct_id, prjct_type, status){
		...
		if(status == "01"){
			url = "/pharmai/chemical/manufacturing/selectManu_Method.do?prjct_id=" + prjct_id + "&prjct_type=" + prjct_type + "status=" + status;
		}else if(status == "02"){
			url = "/pharmai/chemical/manufacturing/selectManu_Process_Map.do";
		}else if(status == "03"){
			url = "/pharmai/chemical/manufacturing/selectManu_PHA.do";
		}else if(status == "04"){
			url = "/pharmai/chemical/manufacturing/selectManu_FMEA.do";
		}else if(status == "05"){
			url = "/pharmai/chemical/manufacturing/selectManu_UNIT_IMG.do";
		}else if(status == "06"){
			url = "/pharmai/chemical/manufacturing/selectManu_CPP_Factor.do";
		}else if(status == "07"){
			url = "/pharmai/chemical/manufacturing/selectManu_CQAs.do";
		}else if(status == "08"){
			url = "/pharmai/chemical/manufacturing/selectManu_Excipent.do";
		}else if(status == "09"){
			url = "/pharmai/chemical/manufacturing/selectManu_Result.do";
		}else{
			return;
		}
	...
	function fnCopyProject(prjct_id, prjct_type, step_cd) {
		...
		if (step_cd == "03" || step_cd == "04" || step_cd == "05"
			...
			if (confirms) {
				...
				$("#currentPage").val("1");
				$("#cform").attr({
					action : "/pharmai/manufacturing/copyPrjct.do",
					method : 'post'
				}).submit();
			}
	...
	function fnChoicePrjct(prjct_id, prjct_type, status){
		...
		jQuery.ajax( {
			type : 'POST',
			dataType : 'json',
			url : '/pharmai/formulation/updateChoicePrjctAjax.do',
			async: false,
			data : req,
	...
	function deletePrjctFunc(){
		...
		if(confirms){
			$("#aform").attr({action : "/pharmai/formulation/deleteFormulation.do", method : 'post'}).submit();
		}
