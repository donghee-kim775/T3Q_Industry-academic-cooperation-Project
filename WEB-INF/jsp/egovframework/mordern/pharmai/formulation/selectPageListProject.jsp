...
<script type="text/javascript">
	...
	function fnGoProject(prjct_id, prjct_type, status) {
		...
		if (status == "01") {
			url = "/pharmai/chemical/formulation/selectInput.do?prjct_id=" + prjct_id
					+ "&prjct_type=" + prjct_type + "status=" + status;
		} else if (status == "02") {
			url = "/pharmai/chemical/formulation/selectRoutes.do";
		} else if (status == "03") {
			url = "/pharmai/chemical/formulation/selectExcipient.do";
		} else if (status == "04") {
			url = "/pharmai/chemical/formulation/selectCQAsList.do";
		} else if (status == "05") {
			url = "/pharmai/chemical/formulation/selectExperiment.do";
		} else if (status == "06") {
			url = "/pharmai/chemical/formulation/selectDoE.do";
		} else {
			return;
		}
	...
	function fnCopyProject(prjct_id, prjct_type, step_cd) {
		...
		if (step_cd == "02" || step_cd == "03" || step_cd == "04"
			...
			if (confirms) {
				...
				$("#currentPage").val("1");
				$("#cform").attr({
					action : "/pharmai/formulation/copyPrjct.do",
					method : 'post'
				}).submit();
			}
	...
	function deletePrjctFunc() {
		...
		if (confirms) {
			$("#aform").attr({
				action : "/pharmai/formulation/deleteFormulation.do",
				method : 'post'
			}).submit();
		}
