		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = true; //원본 true;
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/multiplePhotoUpload.do'; 	//다중 파일 업로드 url
	    	...
	    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
	    		...
	    		try{
		    		if(!!tempFile){
		    			...
		    			//fn_callAjaxHTML5(tempFile,sUploadURL);
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	...
    	rqst.open('POST', sUploadURL, true);
    	...
    	rqst.onreadystatechange = function(e) {
    		if (rqst.readyState === XMLHttpRequest.DONE) {
			    ...
			    if(rqst.status === 200) {
			    	if(rqst.response.indexOf('NOTALLOW_') > -1){
			    		var sFileName = rqst.replace("NOTALLOW_", "");
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			var $openerData = opener.parent.document.getElementById("imageBox");
			
			for(var i = 0; i < oFileInfo.length; i++){
				var $imgBoxAdd = $('<input type="hidden" name="imgPath" class="imgPath" id="imgPath" value="'
