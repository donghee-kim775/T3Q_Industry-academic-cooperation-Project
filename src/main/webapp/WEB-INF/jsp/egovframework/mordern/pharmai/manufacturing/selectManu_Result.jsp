...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		var param = {
 		...
 		if(step_new == 'Y'){
			...
			$.ajax({
				url: "/pharmai/manufacturing/getApi7Ajax.do",
				type: 'post',
				data: param,
				async : false,
				success: function(response){
					...
					if(code == "000"){
						...
						var redirectUrl = "/pharmai/chemical/manufacturing/selectManu_Excipent.do?prjct_id="+prjct_id;;
						switch (code){
							//스텝5
							case '001':
							case '002':
	...
	$(document).on("click","#findSelect", function(){
	 	...
	 	var result =
		{
	 	...
	 	setTimeout(function(){
	 		...
	 		$.ajax({
				url: "/pharmai/manufacturing/getApi8Ajax.do",
				type: 'post',
				data: result,
				async : false,
				success: function(response){
					...
					if(code == "000"){
						...
						var prjct_id = $('input[name=prjct_id]').val();
						var msg = response.resultStats.resultData.msg;
						var redirectUrl = "/pharmai/chemical/formulation/selectManu_Result.do?prjct_id="+prjct_id;
						switch (code){
							//스텝5
							case '001':
							case '002':
	...
	function excelDownload(){
		...
		$('#aform').attr({action:'/pharmai/manufacturing/excelDown.do'}).submit();
	}
