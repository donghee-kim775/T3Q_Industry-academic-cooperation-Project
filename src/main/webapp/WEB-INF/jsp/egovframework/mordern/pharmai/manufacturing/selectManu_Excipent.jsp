...
//<![CDATA[
	...
	$(document).ready(function(e){
		console.log("dsafdsafsdafsdafsdafasdf");
		...
		if(step_new == 'Y'){
			$.ajax({
				url : '/pharmai/manufacturing/getApi6Ajax.do',
				type : 'post',
				dataType : 'json',
				data : param,
				async : false,
				success: function(response){
					...
					if(code == "000"){
						...
						var redirectUrl = "/pharmai/chemical/manufacturing/";
						switch (code){
							//스텝3
							case '010':
							case '011':
	...
	function temporarySave(){
		if($("[name=next_data]").val() == "Y"){
			if(confirm("이전에 등록한 실험 data가 존재합니다. 이전 실험data를 초기화 하시겠습니까?")){
				$('#aform').attr({action:'/pharmai/manufacturing/saveTemporary.do'}).submit();
			}
		}else{
			$('#aform').attr({action:'/pharmai/manufacturing/saveTemporary.do'}).submit();
		}
