...
<script type="text/javascript">
	...
	function fnGoPage(currentPage) {
		...
		$("#aform").attr({action : "/admin/stats/selectPageListErrorEventLog.do", method : 'post'
		}).submit();
	}
									...
									<tbody>
										<c:forEach var="item" items="${resultList}" varStatus="status">
											<tr>
												...
												<td class="text-left">${item.REQUST_URL}</td>
												<td class="text-center">
													<button type="button" id="PARAM" class="btn bg-gradient-secondary modal-btn btn-sm" data-param-val='${item.PARAM_VALUE}' data-toggle="modal" data-target="#modal-open" style="margin-right: 5px">PARAM</button>
													<button type="button" id="ERROR" class="btn bg-gradient-danger modal-btn btn-sm" data-param-val='${item.ERROR_VALUE}' data-toggle="modal" data-target="#modal-open">ERROR</button>
												</td>
	...
	<script>
		...
		$(document).ready(function() {
			$('.modal-btn').on('click', function() {
				...
				$(".modal-header p").text($(this).attr('id'));
			});
		});
	</script>
</div>
