...
//<![CDATA[
	...
	function f_GoInsert(){
		...
		if(confirm("등록하시겠습니까?")){
			...
			$.ajax({
				url:"/admin/recsroom/"+fnSysMappingCode()+"insertRecsroomMgt.do",
				data:formData,
				type:'POST',
	...
	/* function fnSysChg(val){
		...
		jQuery.ajax( {
			type : 'POST',
			dataType : 'json',
			url : '/admin/recsroom/'+fnSysMappingCode()+'selectCntntIdSeqAjax.do',
			data : req,
			success : function(param) {
	...
	function fnGoPreview(device){
		...
		var url = "/admin/recsroom/selectCNTSPreview.do";
