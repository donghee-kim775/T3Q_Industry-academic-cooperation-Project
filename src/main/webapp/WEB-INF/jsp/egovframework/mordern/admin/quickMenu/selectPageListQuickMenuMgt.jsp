...
<script type="text/javascript">
	...
	$(function() {
		//검색 onchange event
		$('[name=sch_sys_code], [name=sch_quick_zone_code]').change(function(e) {
			fnSearch();
		});
	...
	function fnGoPage(currentPage) {
		...
		$("#aform").attr({action:"/admin/quickMenu/"+fnSysMappingCode()+"selectPageListQuickMenuMgt.do", method:'post'}).submit();
	}

	//등록
	function fnInsertForm() {
		document.location.href="/admin/quickMenu/"+fnSysMappingCode()+"insertFormQuickMenuMgt.do";
	}

	//상세조회
	function fnSelect(quickMenu_seq,sch_sys_code, sch_quick_zone_code) {
		$("#quickMenu_seq").val(quickMenu_seq);
...
<div class="content">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<form role="form" id="aform" method="post" action="/admin/quickMenu/selectPageListQuickMenuMgt.do">
					<input type="hidden" id="quickMenu_seq" name="quickMenu_seq"/>
						...
						<div class="card-header">
							<div class="form-row justify-content-between">
								...
								<div class="form-inline">
									...
									<select id="sch_quick_zone_code" name="sch_quick_zone_code" class="form-control form-control-sm mr-1 mb-1">
										<option value="">[코드]&nbsp;&nbsp;퀵메뉴구역</option>
										<c:forEach items="${zoneComboStr}" var="data">
											<option value="${data.CODE}"
												<c:if test="${param.sch_quick_zone_code eq data.CODE}">selected="selected"</c:if>>[${data.CODE}]&nbsp;&nbsp;${data.CODE_NM}</option>
										</c:forEach>
									</select>
									<select id="search_list_st" name="sch_type" class="form-control form-control-sm mr-1 mb-1">
										<option value="quickMenu_nm"
											<c:if test="${requestScope.param.sch_type == 'quickMenu_nm'}">selected="selected"</c:if>>퀵메뉴 명</option>
										<option value="attrb_1"
											<c:if test="${requestScope.param.sch_type == 'attrb_1'}">selected="selected"</c:if>>속성 1</option>
										<option value="attrb_2"
											<c:if test="${requestScope.param.sch_type == 'attrb_2'}">selected="selected"</c:if>>속성 2</option>
										<option value="attrb_3"
											<c:if test="${requestScope.param.sch_type == 'attrb_3'}">selected="selected"</c:if>>속성 3</option>
										<option value="attrb_123"
											<c:if test="${requestScope.param.sch_type == 'attrb_123'}">selected="selected"</c:if>>속성 1+속성 2+속성 3</option>
									</select>
									<div class="input-group input-group-sm mr-1 mb-1">
										<input type="text" class="form-control form-control-sm" id="sch_text" name="sch_text" title="검색어를 입력하세요." placeholder="검색어를 입력하세요." onKeydown="if(event.keyCode==13){fnSearch(); return false;}" value="${requestScope.param.sch_text}" />
							...
							<div class="table-responsive">
								<table class="table table-hover text-nowrap table-grid">
									...
									<tr>
											...
											<th class="text-center">퀵메뉴구역코드</th>
											<th class="text-center">퀵메뉴구역</th>
											<th class="text-center">퀵메뉴 명</th>
									...
									<tbody>
										...
										<tr style="cursor: pointer; cursor: hand;" onclick="fnSelect('${item.QUICK_MENU_SEQ}', '${item.sch_sys_code}', '${item.sch_quick_zone_code}')">
												...
												<td class="text-center txt_overflow">${item.QUICK_MENU_NM}</td>
